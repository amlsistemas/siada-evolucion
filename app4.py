...
def calcular_hora_fin(hora_inicio, duracion_horas):
    """Calcula la hora de fin basada en hora de inicio y duración en horas."""
    try:
        h, m = map(int, hora_inicio.split(":"))
        inicio_dt = datetime.datetime.combine(datetime.date.today(), datetime.time(h, m))
        fin_dt = inicio_dt + datetime.timedelta(hours=duracion_horas)
        return fin_dt.strftime("%H:%M")
    except Exception:
        return "12:00"
...
def cargar_archivos_basic(uploaded_grupos, uploaded_instructores, uploaded_ambientes, uploaded_curriculo):
    ...
    try:
        ...
        # =====================================================
        col_ficha = None
        for col in grupos_df.columns:
            if any(kw in col.lower() for kw in ["grupo", "ficha"]):
                col_ficha = col
                break
        if col_ficha is None:
            col_ficha = grupos_df.columns[0]
        ...
        for _, row in grupos_df.iterrows():
            ...
            for key, candidates in col_map_grupos.items():
                ...
                for cand in candidates:
                    ...
                    if matches:
                        candidate_value = safe_text(row.get(matches[0]))
        ...
        # =====================================================
        col_instructor = None
        for col in instructores_df.columns:
            if any(kw in col.lower() for kw in ["nombre", "instructor", "docente"]):
                col_instructor = col
                break
        if col_instructor is None:
            col_instructor = instructores_df.columns[0]
        ...
        for _, row in instructores_df.iterrows():
            ...
            for col in instructores_df.columns:
                cl = col.lower()
                if any(kw in cl for kw in ['jornada', 'turno']):
                    inst['Jornada del Instructor'] = safe_text(row.get(col))
                if any(kw in cl for kw in ['exclusión', 'restriccion', 'no disponible', 'exclu']):
                    inst['Exclusiones del Instructor'] = safe_text(row.get(col))
                if any(kw in cl for kw in ['municipio', 'ciudad', 'sede']):
                    inst['Municipio'] = safe_text(row.get(col))
        ...
        # =====================================================
        col_ambiente = None
        for col in ambientes_df.columns:
            if any(kw in col.lower() for kw in ['ambiente', 'aula', 'salón', 'sala', 'laboratorio']):
                col_ambiente = col
                break
        if col_ambiente is None:
            col_ambiente = ambientes_df.columns[0]
                ...
                for cand in candidates:
                    ...
                    if matches:
                        ...
                        if value:
                            sess[key] = value
                            break
                if key not in ['hora_inicio', 'hora_fin', 'horas'] and key not in sess:
                    sess[key] = "N/A"
        ...
        # Horas por asignación: preferimos la columna horas; si no, deducimos de hora fin-inicio
        ...
        for sess in curriculo_sessions:
            tri = safe_text(sess.get('trimestre', '1'), default="1")
            curriculo_por_trimestre.setdefault(tri, []).append(sess)

        for tri, sesiones in curriculo_por_trimestre.items():
            st.info(f"Trimestre {tri}: {len(sesiones)} sesiones encontradas")
...
def generar_horario_optimo(
    ...
    curriculo_por_trimestre = curriculo_por_trimestre or {}
    ...
    if num_grupos > num_instructores:
        st.warning(f"⚠️ Más grupos ({num_grupos}) que instructores ({num_instructores}).")
            ...
            for g in range(num_grupos):
                for i in range(num_instructores):
                    if solver.Value(asignacion[(i, d, g)]) == 1:
                        ...
                        grupo_trimestre = safe_text(grupo_info.get('Trimestre', '1'), default="1")
